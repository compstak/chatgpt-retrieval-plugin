...
from chatgpt_retrieval_plugin.api.schemas.models import DocumentChunk, DocumentChunkMetadata, DocumentChunkWithScore, DocumentMetadataFilter, QueryResult, QueryWithEmbedding, Source
from chatgpt_retrieval_plugin.config.settings import settings
from chatgpt_retrieval_plugin.data.datastore import DataStore
from chatgpt_retrieval_plugin.data.schemas import SCHEMA_V1, SCHEMA_V2, Required
from chatgpt_retrieval_plugin.features.date import to_unix_timestamp
...
class MilvusDataStore(DataStore):
    def __init__(
        ...
        self._consistency_level = settings.MILVUS_CONSISTENCY_LEVEL or consistency_level
        self._create_connection()

        self._create_collection(settings.MILVUS_COLLECTION, create_new)  # type: ignore
        self._create_index()
    ...
    def _create_connection(self):
        try:
            ...
            for x in connections.list_connections():
                ...
                if x[1] and ("address" in addr) and (addr["address"] == f"{settings.MILVUS_HOST}:{settings.MILVUS_PORT}"):
                    self.alias = x[0]
                    self._print_info(
            ...
            if len(self.alias) == 0:
                ...
                connections.connect(
                    ...
                    user=settings.MILVUS_USER,  # type: ignore
                    password=settings.MILVUS_PASSWORD,  # type: ignore
                    secure=settings.MILVUS_USE_SECURITY,
                )
                self._print_info(
    ...
    def _create_index(self):
        ...
        self.index_params = settings.MILVUS_INDEX_PARAMS or None
        self.search_params = settings.MILVUS_SEARCH_PARAMS or None
        try:
            ...
            if len(self.col.indexes) == 0:
                if self.index_params is not None:
                    ...
                    self._print_info(
                        f"Create Milvus index: {self.index_params}",
                    )
            ...
            # If an index already exists, grab its params
                ...
                for index in self.col.indexes:
                    idx = index.to_dict()
                    if idx["field"] == settings.EMBEDDING_FIELD:
                        self._print_info(f"Index already exists: {idx}")
            ...
            self._print_info(f"Milvus search parameters: {self.search_params}")
        except Exception as e:
            self._print_err(f"Failed to create index, error: {e}")
    ...
    async def _upsert(
        ...
        try:
            ...
            batches = [insert_data[i : i + settings.UPSERT_BATCH_SIZE] for i in range(0, len(insert_data), settings.UPSERT_BATCH_SIZE)]
            ...
            self._print_err(f"Failed to insert records, error: {e}")
            return []
        ...
        async def _single_query(query: QueryWithEmbedding) -> QueryResult:
            try:
                ...
                res = self.col.search(
                    ...
                    anns_field=settings.EMBEDDING_FIELD,
                    param=self.search_params,
                    limit=query.top_k,
                    expr=filter,
                    output_fields=[field[0] for field in self._get_schema()[return_from:]],  # Ignoring pk, embedding
                )
                ...
                self._print_err(f"Failed to query, error: {e}")
    ...
    async def delete(
        ...
        try:
            ...
            if (ids is not None) and len(ids) > 0:
                ...
                self._print_info(
                    f"Apply {len(pks):d} deletions to schema {self._schema_ver:s}",
                )
                while len(pks) > 0:
            ...
            self._print_err(f"Failed to delete by ids, error: {e}")
        ...
        try:
            ...
            self._print_err(f"Failed to delete by filter, error: {e}")

        self._print_info(f"{delete_count:d} records deleted")
